...
const msg = "Olá, mundo! Vamos codificar?"
const key = "FOGO"
...
/* Retorna o código gerado pela mensagem e chave dadas como argumento
    Utiliza a codificação URI para codificar caracteres especiais (utilizados em URLs)
*/
function encoderMsg(msg, key) {
    let code = "", nonAlphabetCount = 0
    for (let i = 0; i < msg.length; i++) {
        if (!(hasInAlphabet(msg[i]))) {
            code += encodeURIComponent(msg[i])
...
/* Retorna a mensagem original com base no código e chave dadas como arguntento
    Decodifica os caracteres URI para visuzaliação da mensagem original
*/
